	...
	public function add_notices(){
		...
		if(!empty($notices) && is_array($notices)){
			...
			foreach($notices as $notice) {
				if ($notice) { 
					if(in_array($notice->code, $n_discarted)) continue 1;
					if(isset($notice->version) && version_compare($notice->version, Framework::RS_REVISION, '<=')) continue 1;
					if(isset($notice->registered)){ //if this is set, only show the notice if the plugin state is the same
						...
						if($registered !== $rs_valid) continue 1;
					}
					if(isset($notice->show_until) && $notice->show_until !== '0000-00-00 00:00:00'){
						if(strtotime($notice->show_until) < time()) continue 1;
					}
					$_n[] = $notice;
				}
			}
